...
taxonomy_abundance <- read.table("taxonomy/1_Total_coverm_contig_count_species_absolute.tsv", header = TRUE, sep = "\t", row.names = 1)

# Calculate taxonomy relative abundance
total_abundance <- colSums(taxonomy_abundance)
relative_taxonomy_abundance <- t(sweep(taxonomy_abundance, 2, total_abundance, "/"))
rowSums(relative_taxonomy_abundance)

# Load the group information table
group <- read.table('taxonomy/2_group.tsv', sep = '\t', header =TRUE)

# Merge the relative abundance table and the sample grouping table
relative_taxonomy_abundance_grouped <- merge(relative_taxonomy_abundance, group, by.x = "row.names", by.y = "ID")
...
write.table(relative_taxonomy_mean_abundance, "taxonomy/3_relative_taxonomy_mean_abundance.tsv", row.names = FALSE, quote = FALSE)
...
α_diversity <- vegan::estimateR(t(taxonomy_abundance))
...
alpha_diversity <- data.frame(
  ...
  Chao1 = α_diversity[2, ],
  ACE = α_diversity[5, ],
  group = c(rep("Pame", 6), rep("Pful", 6), rep("Esin", 6), rep("Bger", 6))
)

# Export the result file
write.table(alpha_diversity, "taxonomy/4_alpha_diversity.tsv", row.names = FALSE, quote = FALSE)

alpha_diversity$group <- factor(alpha_diversity$group, levels = c("Esin", "Bger", "Pame", "Pful"))

alpha_diversity_zscore <- 
  alpha_diversity %>% 
  dplyr::select(-c("Sample", "group")) %>% 
  scale()

alpha_diversity[2:6] <- alpha_diversity_zscore
...
alpha_diversity_group$`alpha diversity index` <- factor(alpha_diversity_group$`alpha diversity index`, levels = rev(c("Richness", "Shannon", "Simpson", "Chao1", "ACE")))

### alpha_diversity_heatmap 
min(alpha_diversity_group$value)
max(alpha_diversity_group$value)

library(ggforce)

alpha_diversity_heatmap <- 
  ggplot(alpha_diversity_group, 
         aes(x = group, y = `alpha diversity index`, fill = value)
  ) +
  geom_tile(
            height = 1,
            color = "white",
  ...
  theme(
    ...
    axis.text.x = element_text(family = "Arial", face = "plain", size = 6, hjust = 0.5, vjust = 0.5, angle = 90, lineheight = 1, color = "black"),
...
ggsave("taxonomy/5_alpha_diversity_heatmap.svg", plot = alpha_diversity_heatmap, width = 4, height = 8, units = "cm", dpi = 300)
# ggsave("taxonomy/5_alpha_diversity_heatmap.tiff", plot = alpha_diversity_heatmap, width = 4, height = 8, units = "cm", dpi = 300)


### alpha_diversity_boxplot
ggplot(alpha_diversity, aes(sample = Shannon)) +
  stat_qq() + 
  stat_qq_line() +
...
alpha_diversity_boxplot <- 
  ...
  theme(
    axis.title.x = element_blank(), 
    axis.title.y = element_text(family = "Arial", face = "bold", size=8, hjust=0.5, vjust=0.5, angle=90, lineheight=2, color="black"),
...
ggsave("taxonomy/6_alpha_diversity_boxplot.svg", plot = alpha_diversity_boxplot, width = 6, height = 6, units = "cm")
# ggsave("taxonomy/6_alpha_diversity_boxplot.tiff", plot = alpha_diversity_boxplot, width = 6, height = 6, units = "cm", dpi = 300)
...
taxonomy_pcoa_result <- merge(taxonomy_pcoa_coords, group,by.x = "row.names", by.y = "ID")
...
taxonomy_pcoa_result$group <- factor(taxonomy_pcoa_result$group, level = c("Esin", "Bger", "Pame", "Pful"))
...
taxonomy_pcoa_p0 <- 
  ...
  theme(
    axis.title.x.bottom = element_text(family = "Arial", face = "plain", size = 12, hjust = 0.25, vjust = 0.5, angle = 0, lineheight = 2, color = "black"),
    axis.title.y.left = element_text(family = "Arial", face = "plain", size = 12, hjust = 0.25, vjust = 1, angle = 90, lineheight = 2, color = "black"),
    axis.text.x = element_blank(),
...
ggsave("taxonomy/8_metagenome_taxonomy_PCoA_PCoA1~PCoA2.svg", plot = taxonomy_pcoa_final, width = 7, height = 7, units = "cm")
# ggsave("taxonomy/8_metagenome_taxonomy_PCoA_PCoA1~PCoA2.tiff", plot = taxonomy_pcoa_final, width = 7, height = 7, units = "cm", dpi = 300)
...
taxonomy_hc_tree <- tidytree::as.phylo(taxonomy_hc)
...
p_tree <-
  ggtree(groupOTU(taxonomy_hc_tree, groupInfo), 
...
ggsave("taxonomy/10_metagenome_taxonomy_Bray_Curtis_Hierarchical_Dendrogram.svg", plot = p_tree, width = 1.5, height = 6.5, units = "cm")
# ggsave("taxonomy/10_metagenome_taxonomy_Bray_Curtis_Hierarchical_Dendrogram.tiff", plot = p_tree, width = 1.5, height = 6.5, units = "cm", dpi = 300)
...
annot_colors <- list(Host_group = c(Esin = Esin_color, Bger = Bger_color, Pame = Pame_color, Pful = Pful_color),
                     Host_habitat = c(Esin = "#751C6DFF", other = paletteer_d("nationalparkcolors::Acadia")[1]))
...
taxonomy_data <- read.table("taxonomy/1_Total_coverm_contig_count_species_absolute.tsv", header = TRUE, sep = "\t", row.names = 1)
sum <- colSums(taxonomy_data) %>% as.vector()
...
# f_Dysgonomonadaceae
Dysgonomonadaceae_abundance <- t(taxonomy_data[taxonomy_data$family %in% c("f_Dysgonomonadaceae"), ] %>% dplyr::select(-c(25:31)))
...
# f_UBA932
UBA932_abundance <- t(taxonomy_data[taxonomy_data$family %in% c("f_UBA932"), ] %>% dplyr::select(-c(25:31)))
...
# f_Bacteroidaceae
Bacteroidaceae_abundance <- t(taxonomy_data[taxonomy_data$family %in% c("f_Bacteroidaceae"), ] %>% dplyr::select(-c(25:31)))
...
# f_Paludibacteraceae
Paludibacteraceae_abundance <- t(taxonomy_data[taxonomy_data$family %in% c("f_Paludibacteraceae"), ] %>% dplyr::select(-c(25:31)))
...
# f_Rikenellaceae
Rikenellaceae_abundance <- t(taxonomy_data[taxonomy_data$family %in% c("f_Rikenellaceae"), ] %>% dplyr::select(-c(25:31)))
...
# f_Tannerellaceae
Tannerellaceae_abundance <- t(taxonomy_data[taxonomy_data$family %in% c("f_Tannerellaceae"), ] %>% dplyr::select(-c(25:31)))
...
# p_Bacteroidota
Bacteroidota_abundance <- t(taxonomy_data[taxonomy_data$phylum %in% c("p_Bacteroidota"), ] %>% dplyr::select(-c(25:31)))
...
# p_Bacillota
Bacillota_abundance <- t(taxonomy_data[taxonomy_data$phylum %in% c("p_Bacillota_A", "p_Bacillota_B", "p_Bacillota_C", "p_Bacillota_I", "p_Bacillota"), ] %>% dplyr::select(-c(25:31)))
...
# p_Pseudomonadota
Pseudomonadota_abundance <- t(taxonomy_data[taxonomy_data$phylum %in% c("p_Pseudomonadota"), ] %>% dplyr::select(-c(25:31)))
...
# p_Desulfobacterota
Desulfobacterota_abundance <- t(taxonomy_data[taxonomy_data$phylum %in% c("p_Desulfobacterota"), ] %>% dplyr::select(-c(25:31)))
...
# p_Planctomycetota
Planctomycetota_abundance <- t(taxonomy_data[taxonomy_data$phylum %in% c("p_Planctomycetota"), ] %>% dplyr::select(-c(25:31)))
...
# p_Actinomycetota
Actinomycetota_abundance <- t(taxonomy_data[taxonomy_data$phylum %in% c("p_Actinomycetota"), ] %>% dplyr::select(-c(25:31)))
...
# p_Verrucomicrobiota
Verrucomicrobiota_abundance <- t(taxonomy_data[taxonomy_data$phylum %in% c("p_Verrucomicrobiota"), ] %>% dplyr::select(-c(25:31)))
...
Dysgonomonadaceae_taxonomy_pcoa_result <- merge(Dysgonomonadaceae_taxonomy_pcoa_coords, group, by.x = "row.names", by.y = "ID")
Dysgonomonadaceae_taxonomy_pcoa_result$facet <- c(rep("Dysgonomonadaceae", 24))
...
UBA932_taxonomy_pcoa_result <- merge(UBA932_taxonomy_pcoa_coords, group, by.x = "row.names", by.y = "ID")
UBA932_taxonomy_pcoa_result$facet <- c(rep("UBA932", 24))
...
Bacteroidaceae_taxonomy_pcoa_result <- merge(Bacteroidaceae_taxonomy_pcoa_coords, group, by.x = "row.names", by.y = "ID")
Bacteroidaceae_taxonomy_pcoa_result$facet <- c(rep("Bacteroidaceae", 24))
...
Paludibacteraceae_taxonomy_pcoa_result <- merge(Paludibacteraceae_taxonomy_pcoa_coords, group, by.x = "row.names", by.y = "ID")
Paludibacteraceae_taxonomy_pcoa_result$facet <- c(rep("Paludibacteraceae", 24))
...
Rikenellaceae_taxonomy_pcoa_result <- merge(Rikenellaceae_taxonomy_pcoa_coords, group, by.x = "row.names", by.y = "ID")
Rikenellaceae_taxonomy_pcoa_result$facet <- c(rep("Rikenellaceae", 24))
...
Tannerellaceae_taxonomy_pcoa_result <- merge(Tannerellaceae_taxonomy_pcoa_coords, group, by.x = "row.names", by.y = "ID")
Tannerellaceae_taxonomy_pcoa_result$facet <- c(rep("Tannerellaceae", 24))
...
Bacteroidota_taxonomy_pcoa_result <- merge(Bacteroidota_taxonomy_pcoa_coords, group, by.x = "row.names", by.y = "ID")
Bacteroidota_taxonomy_pcoa_result$facet <- c(rep("Bacteroidota", 24))
...
Bacillota_taxonomy_pcoa_result <- merge(Bacillota_taxonomy_pcoa_coords, group, by.x = "row.names", by.y = "ID")
Bacillota_taxonomy_pcoa_result$facet <- c(rep("Bacillota", 24))
...
Pseudomonadota_taxonomy_pcoa_result <- merge(Pseudomonadota_taxonomy_pcoa_coords, group, by.x = "row.names", by.y = "ID")
Pseudomonadota_taxonomy_pcoa_result$facet <- c(rep("Pseudomonadota", 24))
...
Desulfobacterota_taxonomy_pcoa_result <- merge(Desulfobacterota_taxonomy_pcoa_coords, group, by.x = "row.names", by.y = "ID")
Desulfobacterota_taxonomy_pcoa_result$facet <- c(rep("Desulfobacterota", 24))
...
Planctomycetota_taxonomy_pcoa_result <- merge(Planctomycetota_taxonomy_pcoa_coords, group, by.x = "row.names", by.y = "ID")
Planctomycetota_taxonomy_pcoa_result$facet <- c(rep("Planctomycetota", 24))
...
Actinomycetota_taxonomy_pcoa_result <- merge(Actinomycetota_taxonomy_pcoa_coords, group, by.x = "row.names", by.y = "ID")
Actinomycetota_taxonomy_pcoa_result$facet <- c(rep("Actinomycetota", 24))
...
Verrucomicrobiota_taxonomy_pcoa_result <- merge(Verrucomicrobiota_taxonomy_pcoa_coords, group, by.x = "row.names", by.y = "ID")
Verrucomicrobiota_taxonomy_pcoa_result$facet <- c(rep("Verrucomicrobiota", 24))
...
total_taxonomy_pcoa_result$group <- factor(total_taxonomy_pcoa_result$group, levels = c("Esin", "Bger", "Pame", "Pful"))
...
# ggsave("taxonomy/25_ridges_PCoA1.tiff", plot = ridges, width = 3, height = 10, units = "cm", dpi = 300) 
...
alluvial_plot$phylum <- factor(alluvial_plot$phylum, levels = c("Desulfobacterota", 
                                                                "Bacillota",
                                                                "Pseudomonadota",
                                                                "Actinomycetota", 
                                                                "Planctomycetota", 
...
alluvial_plot_long <- alluvial_plot %>% pivot_longer(cols = -phylum, names_to = "group", values_to = "abundance")

alluvial_plot_long$group <- factor(alluvial_plot_long$group, levels = c("Esin", "Bger", "Pame", "Pful"))

alluvial_plot <- 
  ggplot(alluvial_plot_long, 
                        aes(x = group, y = abundance, alluvium = phylum, stratum = phylum)
  ) +
...
total_richness_result$facet <- c(rep("Desulfobacterota", 24), 
                                 rep("Bacillota", 24),
                                 rep("Pseudomonadota", 24),
                                 rep("Actinomycetota", 24), 
                                 rep("Planctomycetota", 24), 
                                 rep("Dysgonomonadaceae", 24), 
                                 rep("UBA932", 24), 
                                 rep("Bacteroidaceae", 24), 
                                 rep("Paludibacteraceae", 24), 
                                 rep("Rikenellaceae", 24), 
                                 rep("Tannerellaceae", 24), 
                                 rep("Bacteroidota", 24),  
                                 rep("Verrucomicrobiota", 24))
...
total_richness_result$group <- c(rep(c(rep("Pame", 6), rep("Pful", 6), rep("Esin", 6), rep("Bger", 6)), 13))

total_richness_result$group <- factor(total_richness_result$group, levels = c("Esin", "Bger", "Pame", "Pful"))
...
### Taxonomy composition stacked bar plot
taxonomy_data_species <- read.table("taxonomy/1_Total_coverm_contig_count_species_absolute.tsv", header = TRUE, sep = "\t", row.names = 1) 
group <- read.table('taxonomy/2_group.tsv', sep = '\t', header = TRUE)
...
# add species name
taxonomy_data_species_relative$Species <- row.names(taxonomy_data_species_relative)

# Convert wide to long
abundance_long <- 
  taxonomy_data_species_relative %>% 
  pivot_longer(-c("Species"), names_to = "sample", values_to = "Abundance")
...
abundance_result <- merge(abundance_long, taxonomy_info, by.x = "Species", by.y = "row.names")
abundance_result <- merge(abundance_result, group, by.x = "sample", by.y = "ID")
abundance_result$group <- factor(abundance_result$group, levels = c("Esin", "Bger", "Pame", "Pful"))
...
top11_species <- total_abundance_species$species[1:12]
top11_species <- top11_species[!top11_species %in% "s_unclassify"]
...
abundance_result_species <- merge(abundance_result_species, group, by.x = "sample", by.y = "ID")
abundance_result_species$group <- factor(abundance_result_species$group, levels = c("Esin", "Bger", "Pame", "Pful"))

# stacked bar plot species
stacked_bar_color <- c("grey40", paletteer_d("ggthemes::Green_Orange_Teal")[2:12])
abundance_result_species$species <- sub("s_", "", abundance_result_species$species)
...
### sample order match to dendrogram sample order
abundance_result_species$sample <- factor(abundance_result_species$sample, 
                                          levels = c("Esin_1", "Esin_2", "Esin_3", "Esin_4", "Esin_5", "Esin_6",
                                                     "Bger_1", "Bger_2", "Bger_3", "Bger_5", "Bger_4", "Bger_6",
                                                     "Pame_3", "Pame_4", "Pame_5", "Pame_6", "Pame_1", "Pame_2",
                                                     "Pful_1", "Pful_6", "Pful_3", "Pful_5", "Pful_2", "Pful_4"))
...
# ggsave("taxonomy/26_taxonomy_Stacked_bar_species.tiff", plot = Stacked_bar_species, width = 5.5, height = 7, units = "cm", dpi = 300) 
...
top11_genus <- total_abundance_genus$genus[1:11]
top11_genus <- top11_genus[!top11_genus %in% "g_unclassify"]
...
# add sample information
abundance_result_genus <- merge(abundance_result_genus, group, by.x = "sample", by.y = "ID")
abundance_result_genus$group <- factor(abundance_result_genus$group, levels = c("Esin", "Bger", "Pame", "Pful"))

# stacked bar plot genus
stacked_bar_color <- c("grey40", paletteer_d("ggthemes::Green_Orange_Teal")[2:12])
abundance_result_genus$genus <- sub("g_", "", abundance_result_genus$genus)
# Make sure that the "family" column is of factor type and that the level order is correct.
...
### sample order match to dendrogram sample order
abundance_result_genus$sample <- factor(abundance_result_genus$sample, 
                                         levels = c("Esin_1", "Esin_2", "Esin_3", "Esin_4", "Esin_5", "Esin_6",
                                                    "Bger_1", "Bger_2", "Bger_3", "Bger_5", "Bger_4", "Bger_6",
                                                    "Pame_3", "Pame_4", "Pame_5", "Pame_6", "Pame_1", "Pame_2",
                                                    "Pful_1", "Pful_6", "Pful_3", "Pful_5", "Pful_2", "Pful_4"))
...
top11_family <- total_abundance_family$family[1:11]
top11_family <- top11_family[!top11_family %in% "f_unclassify"]
abundance_result_family <- 
  abundance_result %>%
  mutate(family = ifelse(family %in% top11_family, family, "others"))

# Re-calculate the abundance for top 10 class
abundance_result_family <- 
  abundance_result_family %>%
...
# add sample information
abundance_result_family <- merge(abundance_result_family, group, by.x = "sample", by.y = "ID")
abundance_result_family$group <- factor(abundance_result_family$group, levels = c("Esin", "Bger", "Pame", "Pful"))

# stacked bar plot family
stacked_bar_color <- c("grey40", paletteer_d("ggthemes::Green_Orange_Teal")[2:12])
abundance_result_family$family <- sub("f_", "", abundance_result_family$family)
# Make sure that the "family" column is of factor type and that the level order is correct.
...
### sample order match to dendrogram sample order
abundance_result_family$sample <- factor(abundance_result_family$sample, 
                                        levels = c("Esin_1", "Esin_2", "Esin_3", "Esin_4", "Esin_5", "Esin_6",
                                                   "Bger_1", "Bger_2", "Bger_3", "Bger_5", "Bger_4", "Bger_6",
                                                   "Pame_3", "Pame_4", "Pame_5", "Pame_6", "Pame_1", "Pame_2",
                                                   "Pful_1", "Pful_6", "Pful_3", "Pful_5", "Pful_2", "Pful_4"))
...
# high light top 11 class
top11_classes <- total_abundance_class$class[1:12]
top11_classes <- top11_classes[!top11_classes %in% "c_unclassify"]
...
# add sample information
abundance_result_class <- merge(abundance_result_class, group, by.x = "sample", by.y = "ID")
abundance_result_class$group <- factor(abundance_result_class$group, levels = c("Esin", "Bger", "Pame", "Pful"))

# stacked bar plot class
abundance_result_class$class <- sub("c_", "", abundance_result_class$class)
...
### sample order match to dendrogram sample order
abundance_result_class$sample <- factor(abundance_result_class$sample, 
                                        levels = c("Esin_1", "Esin_2", "Esin_3", "Esin_4", "Esin_5", "Esin_6",
                                                   "Bger_1", "Bger_2", "Bger_3", "Bger_5", "Bger_4", "Bger_6",
                                                   "Pame_3", "Pame_4", "Pame_5", "Pame_6", "Pame_1", "Pame_2",
                                                   "Pful_1", "Pful_6", "Pful_3", "Pful_5", "Pful_2", "Pful_4"))
...
Stacked_bar_class <- 
  ggplot(abundance_result_class, 
         aes(y = Abundance, x = sample, fill = class)
  ) +
  geom_bar(stat = "identity",
           position = "fill",
           width = 0.6, 
...
ggsave("taxonomy/32_taxonomy_Stacked_bar_class.svg", plot = Stacked_bar_class, width = 5.5, height = 7, units = "cm", dpi = 300)
# ggsave("taxonomy/32_taxonomy_Stacked_bar_class.tiff", plot = Stacked_bar_class, width = 5.5, height = 7, units = "cm", dpi = 300) 
...
top11_order <- total_abundance_order$order[1:12]
top11_order <- top11_order[!top11_order %in% "o_unclassify"]
...
# add sample information
abundance_result_order <- merge(abundance_result_order, group, by.x = "sample", by.y = "ID")
abundance_result_order$group <- factor(abundance_result_order$group, levels = c("Esin", "Bger", "Pame", "Pful"))

# stacked bar plot order
stacked_bar_color <- c("grey40", paletteer_d("ggthemes::Green_Orange_Teal")[2:12])
abundance_result_order$order <- sub("o_", "", abundance_result_order$order)
# Make sure that the "family" column is of factor type and that the level order is correct.
...
### sample order match to dendrogram sample order
abundance_result_order$sample <- factor(abundance_result_order$sample, 
                                        levels = c("Esin_1", "Esin_2", "Esin_3", "Esin_4", "Esin_5", "Esin_6",
                                                   "Bger_1", "Bger_2", "Bger_3", "Bger_5", "Bger_4", "Bger_6",
                                                   "Pame_3", "Pame_4", "Pame_5", "Pame_6", "Pame_1", "Pame_2",
                                                   "Pful_1", "Pful_6", "Pful_3", "Pful_5", "Pful_2", "Pful_4"))
...
ggsave("taxonomy/33_taxonomy_Stacked_bar_order.svg", plot = Stacked_bar_order, width = 5.5, height = 7, units = "cm", dpi = 300)
# ggsave("taxonomy/33_taxonomy_Stacked_bar_order.tiff", plot = Stacked_bar_order, width = 5.5, height = 7, units = "cm", dpi = 300) 
...
top11_phylums <- total_abundance_phylum$phylum[1:12]
top11_phylums <- top11_phylums [!top11_phylums %in% "p_unclassify"]
abundance_result_phylum <- 
  abundance_result %>%
  mutate(phylum = ifelse(phylum %in% top11_phylums, phylum, "others"))

# Re-calculate the abundance for top 11 phylum
abundance_result_phylum <- 
  abundance_result_phylum %>%
...
# add sample information
abundance_result_phylum <- merge(abundance_result_phylum, group, by.x = "sample", by.y = "ID")
abundance_result_phylum$group <- factor(abundance_result_phylum$group, levels = c("Esin", "Bger", "Pame", "Pful"))

### stacked bar plot phylum
stacked_bar_color2 <- c("grey40", paletteer_d("MoMAColors::Klein")[2:12])
abundance_result_phylum$phylum <- sub("p_", "", abundance_result_phylum$phylum)
...
### sample order match to dendrogram sample order
abundance_result_phylum$sample <- factor(abundance_result_phylum$sample, 
                                                           levels = c("Esin_1", "Esin_2", "Esin_3", "Esin_4", "Esin_5", "Esin_6",
                                                                      "Bger_1", "Bger_2", "Bger_3", "Bger_5", "Bger_4", "Bger_6",
                                                                      "Pame_3", "Pame_4", "Pame_5", "Pame_6", "Pame_1", "Pame_2",
                                                                      "Pful_1", "Pful_6", "Pful_3", "Pful_5", "Pful_2", "Pful_4"))
...
### all level stacked barplot legend
...
LDA_score$enrichment <- factor(LDA_score$enrichment, levels = c( "Pful", "Pame", "Bger", "Esin"))
...
LDA_score$species <- gsub("^s_", "", LDA_score$species)
...
LDA_score$class <- gsub("^c_", "", LDA_score$class)
...
# ggsave("taxonomy/37_LDA_score_plot.tiff", plot = LDA_plot, width = 10, height = 6, units = "cm", dpi = 300)
...
ggsave("taxonomy/38_LDA_score_plot_legend.svg", plot = LDA_plot_legend, width = 10, height = 10, units = "cm", dpi = 300)
...
species <- as.data.frame(read_excel("taxonomy/36_lefse_res.xlsx", sheet = 3))
