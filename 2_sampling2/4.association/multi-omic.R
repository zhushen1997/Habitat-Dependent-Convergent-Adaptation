...
MetaG_MetaB_cor_matrix <- MetaG_MetaB_Correlation$r[1:20, 21:40]
MetaG_MetaB_pvalue_matrix <- MetaG_MetaB_Correlation$P[1:20, 21:40]

MetaB_HostT_cor_matrix <- MetaB_HostT_Correlation$r[1:20, 21:40]
MetaB_HostT_pvalue_matrix <- MetaB_HostT_Correlation$P[1:20, 21:40]
...
MetaG_MetaB_data$MetaG <- factor(MetaG_MetaB_data$MetaG, levels = colnames(metagenome))
MetaG_MetaB_data$MetaB <- factor(MetaG_MetaB_data$MetaB, levels = rev(colnames(metabolome)))

MetaB_HostT_data$HostT <- factor(MetaB_HostT_data$HostT, levels = colnames(transcriptome))
MetaB_HostT_data$MetaB <- factor(MetaB_HostT_data$MetaB, levels = rev(colnames(metabolome)))
...
metagenome_info$module <- factor(metagenome_info$module, levels = metagenome_info$module)
...
transcriptome_info$pathway <- factor(transcriptome_info$pathway, levels = transcriptome_info$pathway)
...
MetaB_legendry <-
  ...
  scale_fill_manual(values = c(paletteer::paletteer_d("LaCroixColoR::Apricot")[1:5], "grey"), drop = FALSE) +
  scale_color_manual(values = c(paletteer::paletteer_d("LaCroixColoR::Apricot")[1:5], "grey"), drop = FALSE) +
  theme_void() +
  theme(legend.position = "none",
        legend.key.height = unit(5, "pt"),
        legend.key.width = unit(5, "pt"),
...
# Extract the correlation matrix and the p-value matrix
lasso_MetaG_MetaB_cor_matrix <- lasso_MetaG_MetaB_Correlation$r[1:34, 35:79]
lasso_MetaG_MetaB_pvalue_matrix <- lasso_MetaG_MetaB_Correlation$P[1:34, 35:79]

lasso_MetaB_HostT_cor_matrix <- lasso_MetaB_HostT_Correlation$r[1:34, 35:71]
lasso_MetaB_HostT_pvalue_matrix <- lasso_MetaB_HostT_Correlation$P[1:34, 35:71]
...
lasso_metagenome_info$label <- paste0("MetaG-", lasso_metagenome_info$module_name)
...
multi_omic <- 
  ...
  mutate(habitat = c(rep("home", 6), rep("wild", 6), rep("home", 12)))
  
multi_omic$group <- factor(multi_omic$group , levels = c("Esin", "Bger", "Pame", "Pful"))

multi_omic$habitat <- factor(multi_omic$habitat, levels = c("home", "wild"))

MetaG <- paste0("MetaG_", colnames(metagenome))

MetaB <- colnames(metabolome)
...
p1 <- 
  ggplot(multi_omic, 
         aes(x = MetaG_M00019, y = MetaB_M183) 
  )+
...
p2 <- 
  ggplot(multi_omic, 
         aes(x = MetaG_M00535, y = MetaB_M183) 
  )+
  geom_point(aes(fill = group, color = group),
             size = 1,
             shape = 21,
...
p3 <- 
  ggplot(multi_omic, 
         aes(x = MetaG_M00570, y = MetaB_M183) 
  )+
  geom_point(aes(fill = group, color = group),
             size = 1,
             shape = 21,
...
p4 <- 
  ggplot(multi_omic, 
         aes(x = MetaG_M00432, y = MetaB_M183) 
  )+
  geom_point(aes(fill = group, color = group),
             size = 1,
             shape = 21,
...
p5 <- 
  ggplot(multi_omic, 
         aes(x = MetaG_M00019, y = MetaB_M134) 
  )+
  geom_point(aes(fill = group, color = group),
             size = 1,
             shape = 21,
...
p6 <- 
  ggplot(multi_omic, 
         aes(y = HostT_ko04150, x = MetaB_M134) 
  )+
  geom_point(aes(fill = group, color = group),
             size = 1,
             shape = 21,
...
p7 <- 
  ggplot(multi_omic, 
         aes(y = HostT_ko04150, x = MetaB_M183) 
  )+
  geom_point(aes(fill = group, color = group),
             size = 1,
             shape = 21,
...
p8 <- 
  ggplot(multi_omic, 
         aes(y = HostT_ko04150, x = HostT_ko00190) 
  )+
  geom_point(aes(fill = group, color = group),
             size = 1,
             shape = 21,
...
p9 <- 
  ggplot(multi_omic, 
         aes(y = HostT_ko04150, x = HostT_ko04392) 
  )+
  geom_point(aes(fill = group, color = group),
             size = 1,
             shape = 21,
...
MetaG_MetaB = p1 / p2 / p3 / p4 / p5

MetaG_MetaB

MetaB_HostT = p6 / p7 / p8/ p9
...
BCAA_module_heatmap_mean_matrix_long <- 
  BCAA_module_heatmap_mean_matrix %>% 
  # pivot_longer(-c("group"), names_to = "module", values_to = "enrichment")
  pivot_longer(-group2, names_to = "module", values_to = "enrichment")

# BCAA_module_heatmap_mean_matrix_long$group <- factor(BCAA_module_heatmap_mean_matrix_long$group, levels = c("Esin", "Bger", "Pame", "Pful"))
BCAA_module_heatmap_mean_matrix_long$group2 <- factor(BCAA_module_heatmap_mean_matrix_long$group2, levels = c("Esin", "Other"))

BCAA_module_heatmap_mean_matrix_long$module <- factor(BCAA_module_heatmap_mean_matrix_long$module, levels = c("M00535", "M00019", "M00570", "M00432"))
...
ggplot2::ggsave("1_multi_omic_association/34_BCAA_module_heatmap.svg", plot = BCAA_module_heatmap, width = 2.2, height = 2.6, units = "cm", dpi = 300)
# ggplot2::ggsave("1_multi_omic_association/34_BCAA_module_heatmap.tiff", plot = BCAA_module_heatmap, width = 2.2, height = 2.6, units = "cm", dpi = 300)
...
key_metagenome_module$group <- factor(rep(c(rep("Bger", 24), rep("Esin", 24), rep("Pame", 24), rep("Pful", 24))), levels = c("Esin", "Bger", "Pame", "Pful"))
...
plot(key_metagenome_module_box)

ggplot2::ggsave("1_multi_omic_association/35_BCAA_metagenome_module_box.svg", plot = key_metagenome_module_box , width = 10, height = 4.25, units = "cm", dpi = 300)
# ggplot2::ggsave("1_multi_omic_association/35_BCAA_metagenome_module_box.tiff", plot = key_metagenome_module_box , width = 10, height = 4.25, units = "cm", dpi = 300)
...
BCAA_KO_abundance_mean <- 
  ...
  group_by(group) %>%
  summarise(across(where(is.numeric), mean, na.rm = TRUE)) %>%
  # mutate(across(where(is.numeric), ~ scales::scale(.x))
  mutate(across(where(is.numeric), ~ scales::rescale(.x, to = c(-2, 2)))) %>%
  mutate(group2 = ifelse(group == "Esin", "Esin", "Other")) %>%
  group_by(group2) %>%
  summarise(across(where(is.numeric), mean, na.rm = TRUE))
...
BCAA_KO_abundance_mean_long <- 
  ...
  pivot_longer(-c("group2"), names_to = "KO", values_to = "scaled TPM")

# BCAA_KO_abundance_mean_long$group <- factor(BCAA_KO_abundance_mean_long$group, levels = c("Esin", "Bger", "Pame", "Pful"))
BCAA_KO_abundance_mean_long$group2 <- factor(BCAA_KO_abundance_mean_long$group2, levels = c("Esin", "Other"))

BCAA_KO_abundance_mean_long$KO <- factor(BCAA_KO_abundance_mean_long$KO, levels = c(
                                                                          "K01754",
                                                                          "K00053",
                                                                          "K01687",
                                                                          "K01652",
...
ggplot2::ggsave("1_multi_omic_association/37_BCAA_KO_abundance_heatmap.svg", plot = BCAA_KO_abundance_heatmap_p, width = 2.2, height = 6, units = "cm", dpi = 300)
# ggplot2::ggsave("1_multi_omic_association/37_BCAA_KO_abundance_heatmap.tiff", plot = BCAA_KO_abundance_heatmap_p, width = 2.2, height = 6, units = "cm", dpi = 300)
...
key_metagenome_KO$group  <- factor(rep(c(rep("Bger", 72), rep("Esin", 72), rep("Pame", 72), rep("Pful", 72))), levels = c("Esin", "Bger", "Pame", "Pful"))
...
BCAA_module_heatmap_mean_matrix_long2$group <- factor(BCAA_module_heatmap_mean_matrix_long2$group, levels = c("Esin", "Bger", "Pame", "Pful"))

BCAA_module_heatmap_mean_matrix_long2$module <- factor(BCAA_module_heatmap_mean_matrix_long2$module, levels = c("M00570", "M00019", "M00432", "M00535"))
...
BCAA_KO_abundance_mean_long2$group <- factor(BCAA_KO_abundance_mean_long2$group, levels = c("Esin", "Bger", "Pame", "Pful"))

BCAA_KO_abundance_mean_long2$KO <- factor(BCAA_KO_abundance_mean_long2$KO, levels = c("K17989",
                                                                                    "K01754",
                                                                                    "K00053",
                                                                                    "K01687",
                                                                                    "K01652",
...
module <- data.frame(Module = c("M00535", "M00432", "M00019", "M00570"),
                     Y = c(9.8, 7.6, 5.4, 3.2))

module$Module <- factor(module$Module, levels = c("M00570", "M00019", "M00432", "M00535"))
...
link <-
  ...
  geom_point(aes(x = 2, y = KO), fill = "grey" , color = "grey", size = 1, shape = 21) +
  # module point
  geom_point(data = module, aes(x = 4, y = Y, fill = Module, color = Module), size = 3, shape = 22, show.legend = F) + 
...
BCAA_metabolome_module <- 
  ...
  as.data.frame() %>% 
  rownames_to_column(var = "sample") %>%
  dplyr::select(c("sample", "M183", "M134")) %>% 
  pivot_longer(-sample, names_to = "module", values_to = "enrich score") %>% 
  mutate(group = factor(c(rep("Bger", 12), rep("Esin", 12), rep("Pame", 12), rep("Pful", 12)), levels = c("Esin", "Bger", "Pame", "Pful"))) %>%
  mutate(group2 = ifelse(group == "Esin", "Esin", "Other"))
...
BCAA_metabolome_module_grouped <- 
  ...
  group_by(module) %>%
  # mutate(across(c(`enrich score`), ~ scales::scale(.x)) %>%
  mutate(across(c(`enrich score`), ~ scales::rescale(.x, to = c(-2, 2)))) %>%
  as.data.frame() %>% 
  mutate(group2 = ifelse(group == "Esin", "Esin", "Other")) %>% 
  group_by(module, group2) %>%
  summarise(across(c(`enrich score`), mean, na.rm = TRUE), .groups = "drop")

min <- min(BCAA_metabolome_module_grouped$`enrich score`)
max <- max(BCAA_metabolome_module_grouped$`enrich score`)
...
BCAA_metabolome_module <- 
  ...
  mutate(module = case_when(
    module == "M183" ~ "MetaB_M183(Leu/Ile)",
    module == "M134" ~ "MetaB_M134(Val)"
  ))
...
BCAA_metabolome_module$top_label <- "BCAAs compound module"
...
BCAA_non_target_Derivative_cpd <- read.table("1_multi_omic_association/42_BCAA_Derivative_PQN_missForest_matrix_noQC.txt", header = T, row.names = 1, sep = "\t")

BCAA_non_target_Derivative_cpd_long <- 
  BCAA_non_target_Derivative_cpd %>% 
  rownames_to_column(var = "CPD_ID") %>% 
  as.data.frame() %>% 
  pivot_longer(-c("MS2_name", "CPD_ID"), names_to = "sample", values_to = "log2intensity")

BCAA_non_target_cpd_long <- 
  BCAA_non_target_Derivative_cpd_long[grepl("CPD0020|CPD0128", BCAA_non_target_Derivative_cpd_long$CPD_ID), ]

BCAA_non_target_cpd_long$group <- c(rep(c(rep("Bger", 6), rep("Esin", 6), rep("Pame", 6), rep("Pful", 6)), 2))
BCAA_non_target_cpd_long$group <- factor(BCAA_non_target_cpd_long$group, levels = c("Esin", "Bger", "Pame", "Pful"))

BCAA_non_target_cpd_long$group2 <- c(rep(c(rep("Other", 6), rep("Esin", 6), rep("Other", 12)), 2))
BCAA_non_target_cpd_long$group2 <- factor(BCAA_non_target_cpd_long$group2, levels = c("Esin", "Other"))

BCAA_non_target_cpd_long$CPD_ID <- factor(BCAA_non_target_cpd_long$CPD_ID, levels = c("CPD0020", "CPD0128"))

BCAA_non_target_cpd_long_grouped <-
  BCAA_non_target_cpd_long %>% 
  mutate(MS2_name = case_when(
    MS2_name == "Isoleucine" ~ "Leucine/Isoleucine",
    MS2_name == "Valine" ~ "Valine"
  )) %>%
...
ggplot2::ggsave("1_multi_omic_association/43_BCAA_non_target_cpd_heatmap.svg", plot = BCAA_non_target_cpd_heatmap , width = 2.2, height = 1, units = "cm", dpi = 300)
# ggplot2::ggsave("1_multi_omic_association/43_BCAA_non_target_cpd_heatmap.tiff", plot = BCAA_non_target_cpd_heatmap , width = 2.2, height = 1, units = "cm", dpi = 300)
...
BCAA_non_target_cpd_long$group2 <- c(rep(c(rep("Other", 6), rep("Esin", 6), rep("Other", 12)), 2))
BCAA_non_target_cpd_long$group2 <- factor(BCAA_non_target_cpd_long$group, levels = c("Esin", "Other"))
...
BCAA_non_target_cpd_long$top_label <- "BCAAs Non-targeted Metabolome"
...
BCAA_non_target_cpd_box_plot <-
  ...
  ggh4x::facet_nested_wrap(~ top_label + MS2_name, 
                           scales = "free_y",
                           ncol = 2
  ) + 
  scale_fill_manual(values = c("Bger" = Bger_color, "Esin" = Esin_color, "Pame" = Pame_color, "Pful" = Pful_color)) +
...
ggplot2::ggsave("1_multi_omic_association/44_BCAA_non_target_cpd_box_plot.svg", plot = BCAA_non_target_cpd_box_plot, width = 5.5, height = 4.25, units = "cm", dpi = 300)
# ggplot2::ggsave("1_multi_omic_association/44_BCAA_non_target_cpd_box_plot.tiff", plot = BCAA_non_target_cpd_box_plot, width = 5.5, height = 4.25, units = "cm", dpi = 300)


### MetaB-M134 cluster and heatmap 
library(tidyverse)
library(Mfuzz)

MetaB_M134_CPD <- 
  read.table("../2.metabolome/2_WGCNA/merge_module/merge_module_gene.txt", header = T, sep = "\t") %>%
  filter(MetaB_module_ID == "MetaB_M134")

MetaB_M134_CPD_metadata <- read_xlsx("../2.metabolome/1_metabolome_matrix/metabolome_metadata.xlsx") %>%
  filter(CPD_ID %in% MetaB_M134_CPD$metabolome_name)

Sample_metadata <- read.csv("../2.metabolome/1_metabolome_matrix/2_group.csv", header = T)

MetaB_M134_CPD_normalize_intensity <- 
  read.csv("../2.metabolome/1_metabolome_matrix/1_metabolome_matrix.csv", header = T) %>%
  filter(CPD_ID %in% MetaB_M134_CPD$metabolome_name)  %>%
...
MetaB_M134_CPD_normalize_intensity$group <- factor(MetaB_M134_CPD_normalize_intensity$group , levels = c("Esin", "Bger", "Pame", "Pful"))

MetaB_M134_CPD_cluter_Trend <- 
  ggplot(MetaB_M134_CPD_normalize_intensity, 
...
print(MetaB_M134_CPD_cluter_Trend)
...
col_dist = dist(t(MetaB_M134_heatmap_mat))
col_hc = hclust(col_dist)
col_dend = as.dendrogram(col_hc)

select_cpds <- c("CPD0020")
row_idx <- which(rownames(MetaB_M134_heatmap_mat) %in% select_cpds)
labels <- c("L-Valine")
...
group_info = colnames(MetaB_M134_heatmap_mat)
...
bottom_text_anno <- HeatmapAnnotation(
  text = anno_text(c("Esin", "Bger", "Pame", "Pful"),
                   location = 0.5,
                   just = "center",
                   rot = 45,
                   gp = gpar(fontsize = 6))
...
draw(MetaB_M134_CPD_heatmap) 



### MetaB-M183 cluster and heatmap 
library(tidyverse)
library(Mfuzz)

MetaB_M183_CPD <- 
  read.table("../2.metabolome/2_WGCNA/merge_module/merge_module_gene.txt", header = T, sep = "\t") %>%
  filter(MetaB_module_ID == "MetaB_M183")

MetaB_M183_CPD_metadata <- read_xlsx("../2.metabolome/1_metabolome_matrix/metabolome_metadata.xlsx") %>%
  filter(CPD_ID %in% MetaB_M183_CPD$metabolome_name)

Sample_metadata <- read.csv("../2.metabolome/1_metabolome_matrix/2_group.csv", header = T)

MetaB_M183_CPD_normalize_intensity <- 
  read.csv("../2.metabolome/1_metabolome_matrix/1_metabolome_matrix.csv", header = T) %>%
  filter(CPD_ID %in% MetaB_M183_CPD$metabolome_name)  %>%
  column_to_rownames("CPD_ID") %>%
  t() %>%
...
MetaB_M183_CPD_normalize_intensity$group <- factor(MetaB_M183_CPD_normalize_intensity$group , levels = c("Esin", "Bger", "Pame", "Pful"))

MetaB_M183_CPD_cluter_Trend <- 
  ggplot(MetaB_M183_CPD_normalize_intensity, 
         aes(x = group, y = Z_Score)
  ) +
  stat_summary(aes(group = 1), 
               fun.data = mean_sdl, 
...
print(MetaB_M183_CPD_cluter_Trend)

library(patchwork)
combined_plot <- MetaB_M183_CPD_cluter_Trend | MetaB_M134_CPD_cluter_Trend

ggplot2::ggsave("1_multi_omic_association/45_BCAA_metabolome_trend_plot.svg", plot = combined_plot, width = 5.5, height = 4.25, units = "cm", dpi = 300)
# ggplot2::ggsave("1_multi_omic_association/45_BCAA_metabolome_trend_plot.tiff", plot = combined_plot, width = 5.5, height = 4.25, units = "cm", dpi = 300)

# heatmap
MetaB_M183_heatmap_mat <- 
  MetaB_M183_CPD_normalize_intensity %>%
  group_by(Compound_ID, group) %>%
  summarise(mean_Z = mean(Z_Score, na.rm = TRUE)) %>%
  mutate(zscore = scales::rescale(mean_Z, to = c(-2, 2))) %>%
...
col_dist = dist(t(MetaB_M183_heatmap_mat))
col_hc = hclust(col_dist)
col_dend = as.dendrogram(col_hc)

select_cpds <- c("CPD0158", "CPD0128")
row_idx <- which(rownames(MetaB_M183_heatmap_mat) %in% select_cpds)
labels <- c("L-Leucine", "L-Isoleucin")
...
group_info = colnames(MetaB_M183_heatmap_mat)
...
draw(MetaB_M183_CPD_heatmap) 
dev.off()

pdf("1_multi_omic_association/46_BCAA_MetaB_Heatmap.pdf", width = 3, height = 3)
draw(MetaB_M134_CPD_heatmap) 
draw(MetaB_M183_CPD_heatmap) 
dev.off()
...
BCAA_target_Derivative_cpd_long <- 
  ...
  mutate(group = factor(rep(c(rep("Esin", 4), rep("Bger", 4), rep("Pame", 4), rep("Pful", 4)), 3), levels = c("Esin", "Bger", "Pame", "Pful"))) %>%
  mutate(group2 = factor(ifelse(group == "Esin", "Esin", "Other"), levels = c("Esin", "Other"))) %>%
...
BCAA_target_Derivative_cpd_long_grouped <-
  ...
  as.data.frame() %>% 
  mutate(group2 = factor(ifelse(group == "Esin", "Esin", "Other"), levels = c("Esin", "Other"))) %>%
  group_by(Metabolite, group2) %>% 
  summarise(across(c(concentration), mean, na.rm = TRUE), .groups = "drop")
  
min <- min(BCAA_target_Derivative_cpd_long_grouped$concentration)
max <- max(BCAA_target_Derivative_cpd_long_grouped$concentration)
...
ggplot2::ggsave("1_multi_omic_association/48_BCAA_target_cpd_heatmap.svg", plot = BCAA_target_cpd_heatmap , width = 2.2, height = 1, units = "cm", dpi = 300)
# ggplot2::ggsave("1_multi_omic_association/48_BCAA_target_cpd_heatmap.tiff", plot = BCAA_target_cpd_heatmap , width = 2.2, height = 1, units = "cm", dpi = 300)


### target metabolome boxplot
library(scales)

BCAA_target_Derivative_cpd_long$top_label <- "BCAAs Targeted Metabolome"
...
BCAA_target_cpd_box_plot <-
  ...
  ggh4x::facet_nested_wrap(~ top_label + Metabolite, 
                           scales = "free_y",
                           ncol = 3
  ) + 
  scale_fill_manual(values = c("Bger" = Bger_color, "Esin" = Esin_color,"Pame" = Pame_color, "Pful" = Pful_color)) +
...
### Transcriptome module & OG 

### transcriptome pathway heatmap
HostT_mTOR_relate_pathway <- 
  read.table("1_multi_omic_association/20_total_e_d_pathway_HostT.tsv", header = T, row.names = 1) %>% 
  t() %>%
  as.data.frame() %>%
...
HostT_mTOR_relate_data <- 
  ...
  mutate(group = c(rep("Bger", 6), rep("Esin", 6), rep("Pame", 6), rep("Pful", 6))) %>%
  group_by(group) %>%
  summarise(across(c(-sample), mean, na.rm = TRUE)) %>%
...
HostT_mTOR_relate_data_long <- 
  HostT_mTOR_relate_data %>%
  # dplyr::select(-ko04150) %>%
  # pivot_longer(-c("group"), names_to = "pathway", values_to = "enrich score")
...
ggplot2::ggsave("1_multi_omic_association/50_HostT_mTOR_relate_pathway_heatmap.svg", plot = HostT_mTOR_relate_pathway_heatmap, width = 2.2, height = 4, units = "cm", dpi = 300)
# ggplot2::ggsave("1_multi_omic_association/50_HostT_mTOR_relate_pathway_heatmap.tiff", plot = HostT_mTOR_relate_pathway_heatmap, width = 2.2, height = 4, units = "cm", dpi = 300)
...
key_transcriptome_pathway_data$group <- factor(key_transcriptome_pathway_data$group, levels = c("Esin", "Bger", "Pame", "Pful"))
...
key_transcriptome_pathway_box_plot <-
  ...
  ggh4x::facet_nested_wrap(~ top_label + pathway, 
                           scales = "free_y",
                           ncol = 2,
                           nrow = 2,
...
ggplot2::ggsave("1_multi_omic_association/51_key_transcriptome_pathway_box.svg", plot = key_transcriptome_pathway_box_plot, width = 6, height = 9, units = "cm", dpi = 300)
# ggplot2::ggsave("1_multi_omic_association/51_key_transcriptome_pathway_box.tiff", plot = key_transcriptome_pathway_box_plot, width = 6, height = 9, units = "cm", dpi = 300)


### Transcriptome key gene

### mTOR relate gene expression heatmap
Gene_ID_2_KO <- read.table("../3.transcriptome/2_downstream_analysis/OG_2_KO.txt", header = T, sep = "\t")

mTOR_relate_gene_exprission_data <- 
  read.table("../3.transcriptome/2_downstream_analysis/1_OG_tpm_salmon_QN.tsv", header = T, sep = "\t") %>% 
  left_join(Gene_ID_2_KO, by = c("gene" = "protein")) %>%
  ...
  dplyr::select(gene, KO, symbol, everything()) %>%
  pivot_longer(-c(gene, KO, symbol), names_to = "sample", values_to = "value") %>%
  mutate(group = rep(c(rep("Pame", 6), rep("Pful", 6), rep("Esin", 6), rep("Bger", 6)), 6)) %>%
...
mTOR_relate_gene_exprission_data$group <- factor(mTOR_relate_gene_exprission_data$group, levels = c("Esin", "Bger", "Pame", "Pful"))
mTOR_relate_gene_exprission_data$symbol <- factor(mTOR_relate_gene_exprission_data$symbol, levels = c("SLC38A9", "mTOR", "S6K", "eIF4B", 'AKT'))
...
Hemolymph_FatBody_BCAAs$group <- factor(Hemolymph_FatBody_BCAAs$group, levels = c("Esin", "Bger", "Pame", "Pful"))
...
### western bar plot(four species of cockroaches)
wb_result <- 
...
wb_result$group <- factor(c(rep("Esin", 3), rep("Bger", 3), rep("Pame", 3), rep("Pful", 3)), levels = c("Esin", "Bger", "Pame", "Pful"))
...
wb_result_long <- 
  wb_result_long %>%
  filter(singnal %in% c("p-S6K/Tubulin", "p-AKT/AKT"))
...
wb_result_long_group$top_label <- "mTORC1 activity of Fat Body"
wb_result_long$top_label <- "mTORC1 activity of Fat Body"
...
wb_box_plot <- 
  ...
  ggh4x::facet_nested_wrap(~ top_label + singnal, 
                           scales = "free_y",
                           ncol = 2
  ) + 
  stat_compare_means(method = "anova",
...
wb_result_long <- 
  wb_result %>%
  dplyr::select(-c(2,3,4,5)) %>%
  pivot_longer(-c(sample, group), names_to = "signal", values_to = "relativa intensity")
...
wb_result_long_group_scale <- 
  ...
  summarise(across(c(`relativa intensity`), list(mean = mean, sd = sd), na.rm = TRUE)) %>%
  group_by(signal) %>% 
  mutate(across(c(`relativa intensity_mean`), ~ scales::rescale(.x, to = c(-2, 2)))) %>%
  mutate(group2 = ifelse(group == "Esin", "Esin", "Other")) %>%
  group_by(signal, group2) %>%
  summarise(across(`relativa intensity_mean`, mean, na.rm = TRUE)) 
...
wb_heatmap <-
  ...
  geom_tile(aes(x = group2, y = signal, fill = `relativa intensity_mean`),
            color = "white",
            height = 1,
            linewidth = 0.45,
            show.legend = T
...
mito_function_data$group <- factor(rep(c(rep("Bger", 4), rep("Esin", 4), rep("Pame", 4), rep("Pful", 4)), 3), levels = c("Esin", "Bger", "Pame", "Pful"))
...
ggplot2::ggsave("1_multi_omic_association/63_mito_function_heatmap.svg", plot = mito_function_heatmap, width = 2.2, height = 2, units = "cm", dpi = 300)
# ggplot2::ggsave("1_multi_omic_association/63_mito_function_heatmap.tiff", plot = mito_function_heatmap, width = 2.2, height = 2, units = "cm", dpi = 300)
...
ko00190 <- read.table("1_multi_omic_association/65_ko00190_KO_tpm.txt", header = T, sep = "\t")
...
scaled_tpm <- 
  ...
  as.data.frame() %>% 
  rownames_to_column(var = "sample") %>%
  mutate(group = c(rep("Pame", 6), rep("Pful", 6), rep("Esin", 6), rep("Bger", 6))) %>%
  group_by(group) %>%
...
ko00190_group$name <- factor(ko00190_group$name, levels = ko00190_group$name)

ko00190_group_long <- 
  ko00190_group %>%
  pivot_longer(-c(1:6), names_to = "group", values_to = "Scaled TPM")

ko00190_group_long$group <- factor(ko00190_group_long$group, levels = c("Esin","Bger","Pame","Pful"))
...
zscore_ko00190[7:30] <-  zscore_tpm

zscore_ko00190$name <- factor(zscore_ko00190$name, levels = zscore_ko00190$name)

zscore_ko00190_long <- 
  zscore_ko00190 %>%
  pivot_longer(-c(1:6), names_to = "sample", values_to = "tpm_z_score")

zscore_ko00190_long$sample <- factor(zscore_ko00190_long$sample, levels = c(paste0("Esin_", 1:6), 
                                                                            paste0("Bger_", 1:6), 
                                                                            paste0("Pame_", 1:6), 
                                                                            paste0("Pful_", 1:6)))

zscore_ko00190_long$group <- factor( rep(c(rep("Pame", 6), rep("Pful", 6), rep("Esin", 6), rep("Bger", 6)) , 43),  levels = c("Esin", "Bger", "Pame", "Pful")) 
...
zscore_ko00190_long <- 
  ...
  mutate(merge_group = factor(merge_group, levels = c("Esin", "Other")))

zscore_ko00190_long_group <- 
  zscore_ko00190_long %>% 
  group_by(name, merge_group) %>%
  summarise(
...
pval_data <- zscore_ko00190_long %>%
  group_by(name) %>%
  summarise(
    esin_vals = list(tpm_z_score[merge_group == "Esin"]),
    other_vals = list(tpm_z_score[merge_group == "Other"]),
...
ko00190_boxplot_p0 <- 
  ...
  geom_point(
    aes(x = name, y = mean_z,  color = merge_group),
    fill = "white",
    stroke = 1,
    shape = 21,
    alpha = 0.75,
  ...
  geom_errorbar(
    aes(x = name, ymin = mean_z - se, ymax = mean_z + se, color = merge_group),
    position = position_dodge(width = 0.9),
    width = 0.75,
    size = 0.5,
    show.legend = F
  ...
  geom_vline(
    xintercept = seq(1.5, length(unique(zscore_ko00190_long$name)) - 0.5, by = 1),
    linetype = "dashed", 
    color = "black", 
    alpha = 1,
    linewidth = 0.1
  ...
  geom_text(
    data = pval_data,
    aes(x = name, y = y_pos, label = p_signif, color = other_higher),
    size = 6,
    angle = 90,
    hjust = 0.5,
    vjust = 0.75,
  ...
  scale_fill_manual(values = c("Esin" = "#648C16FF", "Other" = "#FF7200FF")) +
  scale_color_manual(
    values = c("Esin" = "#648C16FF", "Other" = "#FF7200FF", "TRUE" = "#FF7200FF", "FALSE" = "#648C16FF")
  ) +
  ...
  theme(
    ...
    axis.text.x = element_text( angle =90, vjust = 0.5, hjust = 1, size = 6, 
                                color = c(rep(paletteer_d("PrettyCols::Bright")[1], 21),
                                          rep(paletteer_d("PrettyCols::Bright")[2], 2),
                                          rep(paletteer_d("PrettyCols::Bright")[3], 3),
                                          rep(paletteer_d("PrettyCols::Bright")[4], 8),
                                          rep(paletteer_d("PrettyCols::Bright")[5], 9))),
...
### main heatmap
ko00190_heatmap_p0 <- 
  ggplot(ko00190_group_long) +
  geom_point(aes(x = name, y = group, size = `Scaled TPM`, fill = Complex, color =  Complex),
             # color = "black",
             stroke = 0.05,
             shape = 21,
             show.legend = T
  ) + 
  annotate("rect", xmin = -Inf, xmax = 21.5, ymin = -Inf, ymax = Inf, fill = paletteer_d("PrettyCols::Bright")[1], color = NA , alpha = 0.10) + 
  annotate("rect", xmin = 0.5, xmax = 21.5, ymin = 4.6, ymax = 5, fill = paletteer_d("PrettyCols::Bright")[1], color = NA , alpha = 1) +
  annotate("text", x = 10.75, y = 4.8, label = c("ETC I"), color = "white", size = 6 / 2.845, hjust = 0.5, vjust = 0.5, fontface = "bold", family = "Arial")+
  annotate("rect", xmin = 21.5, xmax = 23.5, ymin = -Inf, ymax = Inf, fill = paletteer_d("PrettyCols::Bright")[2], color = NA , alpha = 0.15) + 
  annotate("rect", xmin = 21.5, xmax = 23.5, ymin = 4.6, ymax = 5, fill = paletteer_d("PrettyCols::Bright")[2], color = NA , alpha = 1) +
  annotate("text", x = 22.5, y = 4.8, label = c("ETC II"), color = "white", size = 6 / 2.845, hjust = 0.5, vjust = 0.5, fontface = "bold", family = "Arial")+
  annotate("rect", xmin = 23.5, xmax = 26.5, ymin = -Inf, ymax = Inf, fill = paletteer_d("PrettyCols::Bright")[3], color = NA , alpha = 0.15) + 
  annotate("rect", xmin = 23.5, xmax = 26.5, ymin = 4.6, ymax = 5, fill = paletteer_d("PrettyCols::Bright")[3], color = NA , alpha = 1) +
  annotate("text", x = 25, y = 4.8, label = c("ETC III"), color = "white", size = 6 / 2.845, hjust = 0.5, vjust = 0.5, fontface = "bold", family = "Arial")+
  annotate("rect", xmin = 26.5, xmax = 34.5, ymin = -Inf, ymax = Inf, fill = paletteer_d("PrettyCols::Bright")[4], color = NA , alpha = 0.15) + 
  annotate("rect", xmin = 26.5, xmax = 34.5, ymin = 4.6, ymax = 5, fill = paletteer_d("PrettyCols::Bright")[4], color = NA , alpha = 1) + 
  annotate("text", x = 30.5, y = 4.8, label = c("ETC IV"), color = "white", size = 6 / 2.845, hjust = 0.5, vjust = 0.5, fontface = "bold", family = "Arial")+
  annotate("rect", xmin = 34.5, xmax = Inf, ymin = -Inf, ymax = Inf, fill = paletteer_d("PrettyCols::Bright")[5], color = NA , alpha = 0.15) + 
  annotate("rect", xmin = 34.5, xmax = Inf, ymin = 4.6, ymax = 5, fill = paletteer_d("PrettyCols::Bright")[5], color = NA , alpha = 1) + 
  annotate("text", x = 39.5, y = 4.8, label = c("ETC V"), color = "white", size = 6 / 2.845, hjust = 0.5, vjust = 0.5, fontface = "bold", family = "Arial")+
  annotate("rect", xmin = 0, xmax = 0.5, ymin = 3.5, ymax = 4.6, fill = Pful_color, color = NA , alpha = 1) +
  annotate("rect", xmin = 0, xmax = 0.5, ymin = 2.5, ymax = 3.5, fill = Pame_color, color = NA , alpha = 1) +
  annotate("rect", xmin = 0, xmax = 0.5, ymin = 1.5, ymax = 2.5, fill = Bger_color, color = NA , alpha = 1) +
  annotate("rect", xmin = 0, xmax = 0.5, ymin = 1.5, ymax = -Inf, fill = Esin_color, color = NA , alpha = 1) +
  annotate("text", x = 0.25, y = 1, label = c("Esin"), color = "white", size = 6 / 2.845, hjust = 0.5, vjust = 0.5, fontface = "bold", family = "Arial", angle = 90) +
  annotate("text", x = 0.25, y = 2, label = c("Bger"), color = "white", size = 6 / 2.845, hjust = 0.5, vjust = 0.5, fontface = "bold", family = "Arial", angle = 90) +
  annotate("text", x = 0.25, y = 3, label = c("Pame"), color = "white", size = 6 / 2.845, hjust = 0.5, vjust = 0.5, fontface = "bold", family = "Arial", angle = 90) +
  annotate("text", x = 0.25, y = 4, label = c("Pful"), color = "white", size = 6 / 2.845, hjust = 0.5, vjust = 0.5, fontface = "bold", family = "Arial", angle = 90) +
  ...
  theme(#axis.text.x  = element_text( angle = 45, vjust = 1, hjust = 1, size = 6, 
    ...
    # axis.text.y = element_text(size = 6, family = "Arial", face = "bold", angle = 0, color = "black"),
    # axis.ticks.y = element_line(linewidth = 0.25, linetype = "solid", lineend = "butt", color = "black"),
  ...
  theme(
    ...
    # panel.grid.major = element_line(color = "grey90", linewidth = 0.25, linetype = "solid"),
  )
...
Gene_ID_2_KO <- read.table("../3.transcriptome/2_downstream_analysis/OG_2_KO.txt", header = T, sep = "\t")
pathway_2_KO <- read.table("../3.transcriptome/1_OG/OG_2_KEGG_pathway.txt", header = T, sep = "\t")
OG_expression_tpm <- read.table("../3.transcriptome/2_downstream_analysis/1_OG_tpm_salmon_QN.tsv", header = T, sep = "\t") 
pathway_ssGSEA <- read.table("../3.transcriptome/1_OG/transcriptome_KEGG_pathway_ssGSEA_matrix.tsv", header = T, sep = "\t")
...
ko04150_gene_exprission <- 
  ...
  mutate(ID = paste0(gene, ":", KO)) %>%
  dplyr::select(-c(gene,KO,pathway)) %>% 
...
ko04150_gene_exprission_long <- 
  ...
  mutate(group = stringr::str_extract(sample, "^[^_]+")) %>%
  group_by(id, group) %>%
...
pathway_hub_gene_heatmap <-
  ko04150_hub_gene_exprission_heatmap %>%
  insert_bottom(ko00190_hub_gene_exprission_heatmap, height = nrow(ko00190_corr_P) / nrow(ko04150_corr_P)) %>%
  insert_bottom(ko04122_hub_gene_exprission_heatmap, height = nrow(ko04122_corr_P) / nrow(ko04150_corr_P)) %>%
  insert_bottom(ko04214_hub_gene_exprission_heatmap, height = nrow(ko04214_corr_P) / nrow(ko04150_corr_P))
