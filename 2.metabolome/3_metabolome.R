...
metabolome_data <- 
  read.table("3_down_stream_analysis/1_metabolome_LOESS_PQN_missForest_matrix_noQC.tsv", header = TRUE, sep = "\t", row.names = 1) %>% 
  t() %>% 
  as.data.frame()

# load group file
group <- read.table("3_down_stream_analysis/2_group.txt", header = T)
# Merge the group information into the metabolite table
metabolome_data_grouped <- merge(group, metabolome_data, by.x = "sample", by.y = "row.names")


# Calculate the mean value of the group
library(dplyr)
metabolome_mean_abundance <- 
  metabolome_data_grouped %>%
  group_by(group) %>%
  summarise(across(where(is.numeric), mean, na.rm = TRUE)) %>%
  column_to_rownames(var = "group") %>%
  t() %>%
  as.data.frame() %>%
  rownames_to_column(var = "CPD")
...
metabolome_pca_result <- merge(metabolome_pca_coords, group, by.x = "row.names", by.y = "sample")
...
metabolome_pca_result$group <- factor(metabolome_pca_result$group, level = c("Wild", "Lab", "Fam", "Res", "Hos"))
...
ggsave("3_down_stream_analysis/5_metabolome_PCA_PC1~PC2.svg", plot = metabolome_pca_p_final, width = 6, height = 6, units = "cm", dpi = 300)
# ggsave("3_down_stream_analysis/5_metabolome_PCA_PC1~PC2.tiff", plot = metabolome_pca_p_final, width = 6, height = 6, units = "cm", dpi = 300)
...
# leaf order
leaf_order <- order.dendrogram(metabolome_dend)
print(leaf_order) 

# leaf order vector
leaf_order_vector <- labels(metabolome_dend)

# Plot dendrogram
dev.off()
pdf("3_down_stream_analysis/8_metabolome_Hierarchical_Dendrogram.pdf", width = 20, height = 20)
...
ggsave("3_down_stream_analysis/9_metabolome_Hierarchical_Dendrogram.svg", plot = p_tree, width = 5, height = 10, units = "cm", dpi = 300)
# ggsave("3_down_stream_analysis/9_metabolome_Hierarchical_Dendrogram.tiff", plot = p_tree, width = 5, height = 10, units = "cm", dpi = 300)
...
row.names(row_annotation) <- row_annotation$sample
...
annot_colors <- list(Host_group = c(Wild = wild_color, Res = res_color, Hos = hos_color, Fam = fam_color, Lab = lab_color))
...
pheatmap(
  ...
  ### Global parameter
  main = "metabolome Hierarchical heatmap",
  scale = "none",
  border_color = NA,
  show_rownames = FALSE,
  show_colnames = FALSE,
...
up_module_heatmap_p <- 
  ...
  annotate("rect", ymin = 40.5, ymax = 50.5, xmin = 0.5, xmax = 39.5, fill = NA, color = "red", linewidth = 0.1, lineend = "square") + 
  annotate("rect", ymin = 0.5, ymax = 30.5, xmin = 39.5, xmax = 92.5, fill = NA, color = "red", linewidth = 0.1, lineend = "square") + 
  ...
  labs(x = "Enriched metabolome modules of Pame from different habitat", 
       fill = "Enrich Z-Score"
  ) +
  theme_void() + 
  theme(
...
group$group2 <- factor(c(rep("Dwelling", 30), rep("Lab", 10), rep("Wild", 10)), levels = c("Wild", "Lab", "Dwelling"))
...
metabolome_up_module$module <- factor(metabolome_up_module$module, levels = rev(metabolome_up_module$module))
...
up_module_module_info$module <- factor(up_module_module_info$module, levels = rev(up_module_module_info$module))
...
down_module_heatmap_p <- 
  ...
  annotate("rect", ymin = 0.5, ymax = 10.5, xmin = 0.5, xmax = 68.5, fill = NA, color = "blue", linewidth = 0.1, lineend = "square") + 
  annotate("rect", ymin = 10.5, ymax = 20.5, xmin = 68.5 , xmax = 102.5, fill = NA, color = "blue", linewidth = 0.1, lineend = "square") + 
  scale_fill_gradient2(low = "lightblue", 
                       mid = "white", 
                       high = "#FF9898FF", 
                       limit = c(min, max),
  ...
  labs(x = "Depleted metabolome modules of Pame from different habitat", 
       fill = "Enrich Z-score"
  ) +
  theme_void() + 
  theme(
...
group$sample <- factor(group$sample, levels = rev(group$sample))
...
down_module_group_p <-
  ...
  scale_fill_manual(values = c("Wild" = wild_color, "Lab" = lab_color, "Fam" = fam_color, "Res" = res_color, "Hos" = hos_color, "Human" = human_color)) +
  theme_void()
...
# plot module pvalue annotatin
metabolome_down_module$module <- factor(metabolome_down_module$module, levels = rev(metabolome_down_module$module))
...
down_module_module_info$module <- factor(down_module_module_info$module, levels = rev(down_module_module_info$module))
...
down_module_module_total_bar_plot <- 
  ...
  geom_bar(aes(x = regulation, y = sum, fill = regulation),
           stat = "identity",
           width = 0.7) +
  geom_label(aes(label = sum, x = regulation, y = 40), size = 3) + 
  scale_fill_manual(values = c("Wild_down" = wild_color, "Lab_down" = lab_color, "Fam_down" = fam_color, "Res" = res_color, "Hos" = hos_color, "Human_down" = human_color)) + 
  scale_y_reverse() + 
  scale_x_discrete(position = "top") + 
  theme_minimal() +
  theme(
...
### top 10 enriched metabolome module
...
top10_enriched_module_p0 <- 
  ...
  theme(
    ...
    axis.text.y = element_text(family = "Arial", face = "plain", size = 6, hjust = 0, vjust = 0.5, angle = 0, lineheight = 1,
                               color = c(rep(human_color,10), rep(wild_color,10))), 
    axis.ticks.x = element_line(linewidth = 0.25, linetype = "solid", lineend = "butt", color = "black"),
    axis.ticks.y = element_line(linewidth = 0.25, linetype = "solid", lineend = "butt", color = "black"),
    axis.ticks.length.x = unit(2, "pt"),
...
top10_enriched_module_p2 <- 
  ...
  annotate("rect", xmin = 0.5, xmax = 10.5, ymin = 10.5, ymax = 20.5, fill = NA, color = "red", linewidth = 0.25, lineend = "square")+
...
top10_depleted_module$regulation <- factor(top10_depleted_module$regulation, levels = c("Wild_down", "Lab_down", "Human_down"))
...
top10_depleted_module_p0 <- 
  ...
  scale_fill_manual(values = c("Wild_down" = wild_color, "Lab_down" = lab_color, "Fam_down" = lab_color, "Res_down" = res_color, "Hos_down" = hos_color, "Human_down" = human_color)) + 
...
up_module_z_scores <- top10_depleted_module[, 12:61] %>% t() %>% scale() %>% t()

top10_depleted_module[, 12:61] <- up_module_z_scores
...
top10_depleted_module_p2 <- 
  ...
  annotate("rect", xmin = 30.5, xmax = 40.5, ymin = 0.5, ymax = 10.5, fill = NA, color = "blue", linewidth = 0.25, lineend = "square")+
  labs(fill = "Enrich Z-score") +
  theme_void() + 
  theme(
    legend.position = "none", 
...
top20_lasso_metabolome <- rbind(head(coef.df, 10), coef.df[24,], tail(coef.df, 10))
...
top20_lasso_metabolome$label <- factor(top20_lasso_metabolome$label, levels = top20_lasso_metabolome$label)

top20_lasso_metabolome$coef <- -top20_lasso_metabolome$coef

write.table(top20_lasso_metabolome, "../4.association/1_multi_omic_association/4_top10_e_d_module_information_MetaB.tsv", row.names = F, quote = F, sep = "\t")
...
write.table(top20_lasso_metabolome_module_WGCNA, "../4.association/1_multi_omic_association/3_top10_e_d_metabolome_module.tsv", sep = "\t", quote = F, row.names = F)
...
write.table(total_lasso_metabolome, "../4.association/1_multi_omic_association/19_total_e_d_module_information_MetaB.tsv", row.names = F, quote = F, sep = "\t")
...
write.table(total_lasso_metabolome_module_WGCNA, "../4.association/1_multi_omic_association/18_total_e_d_metabolome_module.tsv", sep = "\t", quote = F, row.names = F)
...
lasso_importance_rank <- 
  ...
  scale_y_discrete(position = "right") +
  scale_x_continuous(limits = c(-5, 5), breaks = c(-5, 0, 5), labels = c(-5, 0, 5)) +
  theme_void() + 
  theme(
...
# ggsave("3_down_stream_analysis/28_quadrant_percent_pie_plot.tiff", plot = quadrant_percent_pie_plot, width = 1, height = 1, units = "cm", dpi = 300)
...
double_volcano_plot <- 
  ggplot(double_volcano_data, 
         aes(y = Human_vs_Wild, x = Lab_vs_Wild , fill = quadrant, color = quadrant)
  ) +
  # xlim(c(-20, 20)) + 
  # ylim(c(-20, 20)) +
  # plot scatter
  ...
  scale_color_manual(values = c("1" = scater_color[1], "2" = scater_color[3], "3" = "pink", "4" = scater_color[9], "5" = "grey80", "6" = scater_color[10], "7" = "lightblue", "8" =  scater_color[4], "9" =  scater_color[2]), drop = FALSE) +
  scale_fill_manual(values = c("1" = scater_color[1], "2" = scater_color[3], "3" = "pink", "4" = scater_color[9], "5" = "grey80", "6" = scater_color[10], "7" = "lightblue", "8" =  scater_color[4], "9" =  scater_color[2]), drop = FALSE) +
...
ggsave("3_down_stream_analysis/29_double_volcano_plot.svg", plot = double_volcano_plot, width = 6, height = 6, units = "cm", dpi = 300)
# ggsave("3_down_stream_analysis/29_double_volcano_plot.tiff", plot = double_volcano_plot, width = 6, height = 6, units = "cm", dpi = 300)
...
# Wild enrich
Wild_venn_dat_up  <- read.delim("2_WGCNA/merge_module/Wild_metabolome_WGCNA_module_each_up.tsv")
...
# Lab enrich
Lab_venn_dat_up  <- read.delim("2_WGCNA/merge_module/Lab_metabolome_WGCNA_module_each_up.tsv")
...
# Human enrich
Human_venn_dat_up  <- read.delim("2_WGCNA/merge_module/Human_metabolome_WGCNA_module_each_up.tsv")
...
# Wild deplete
Wild_venn_dat_down  <- read.delim("2_WGCNA/merge_module/Wild_metabolome_WGCNA_module_each_down.tsv")
...
# Lab deplete
Lab_venn_dat_down  <- read.delim("2_WGCNA/merge_module/Lab_metabolome_WGCNA_module_each_down.tsv")
...
# Human deplete
Human_venn_dat_down  <- read.delim("2_WGCNA/merge_module/Human_metabolome_WGCNA_module_each_down.tsv")
...
PLS_DA_model <- opls(metabolome_data, factor(group$group), orthoI = 0) # group is a factor vector; orthoI = 0 : PLS-DA;  orthoI = 1 : OPLS-DA;

plot(PLS_DA_model)

PLS_DA_plot_data <- PLS_DA_model@scoreMN %>% as.data.frame() %>% mutate(sample = rownames(PLS_DA_model@scoreMN), group  = c(rep("Wild", 10), rep("Lab", 10), rep("Fam", 10),  rep("Res", 10), rep("Hos", 10)))

PLS_DA_plot_data$group <- factor(PLS_DA_plot_data$group, levels = c("Wild", "Lab", "Fam", "Res", "Hos"))
