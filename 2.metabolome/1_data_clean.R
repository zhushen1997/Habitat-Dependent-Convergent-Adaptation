...
### Due to the small initial weight and significant heterogeneity of each non-target metabolite sample, a uniform weight was not adopted for compound extraction. 
### Instead, the peak areas were normalized according to the initial weight.
### normalization based on sample weight
raw_intensity_data <- read.table("1_metabolome_matrix/raw_intensity.txt", header = T, row.names = 1) %>% dplyr::select(-matches("QC", ignore.case = TRUE)) 

QC <- read.table("1_metabolome_matrix/raw_intensity.txt", header = T, row.names = 1) %>% dplyr::select(matches("QC", ignore.case = TRUE)) 

colnames(raw_intensity_data)

sample_weight <- read.table("1_metabolome_matrix/weight.txt", header = T, row.names = 1, sep = "\t")
sample_weight <- sample_weight[colnames(raw_intensity_data), , drop = FALSE]

norm_intensity <- sweep(raw_intensity_data, 2, sample_weight$weight_mg, "/") * median(sample_weight$weight_mg) 
norm_intensity <- norm_intensity %>% as.data.frame() %>% rownames_to_column(var = "ID")
norm_intensity_QC <- cbind(norm_intensity, QC) 

write.table(norm_intensity, "1_metabolome_matrix/norm_intensity.txt", quote = FALSE, row.names = F, sep = "\t")
write.table(norm_intensity_QC, "1_metabolome_matrix/norm_intensity_QC.txt", quote = FALSE, row.names = F, sep = "\t")

### 1. load data
peak_df <- read.table("1_metabolome_matrix/norm_intensity_QC.txt", header = T)
meta_df <- read.table("1_metabolome_matrix/group.txt", header = T)
...
peak_se <- as.metabData(e_data = peak_df,
                        f_data = meta_df,
                        edata_cname = "ID",     # The column name of metabolite ID in e_data
...
summary(moleculeFilt, min_num = 25)
plot(moleculeFilt, min_num = 25)
peak_se_filtered <- applyFilt(filter_object = moleculeFilt, omicsData = peak_se_filtered, min_num = 25)
...
filtered_df$ID <- rownames(filtered_df)
...
### 5.1 KNN
knn_raw <- peak_se_log_final$e_data %>%
  tibble::remove_rownames() %>%
  tibble::column_to_rownames(var = "ID") %>%
  as.matrix()
...
knn_imp_noQC <- knn_imp %>%  dplyr::select(-contains("QC"))
...
expr_imp_noQC <- expr_imp %>% dplyr::select(-contains("QC"))

### 7. save results
write.table(knn_imp, file = "1_metabolome_matrix/metabolome_LOESS_PQN_KNN_matrix.tsv", sep = "\t", quote = F, row.names = FALSE)
write.table(knn_imp_noQC, file = "1_metabolome_matrix/metabolome_LOESS_PQN_KNN_matrix_noQC.tsv", sep = "\t", quote = F, row.names = FALSE)

write.table(expr_imp, file = "1_metabolome_matrix/metabolome_LOESS_PQN_missForest_matrix.tsv", sep = "\t", quote = F, row.names = FALSE)
write.table(expr_imp_noQC, file = "1_metabolome_matrix/metabolome_LOESS_PQN_missForest_matrix_noQC.tsv", sep = "\t", quote = F, row.names = FALSE)
write.table(expr_imp_noQC, file = "3_down_stream_analysis/1_metabolome_LOESS_PQN_missForest_matrix_noQC.tsv", sep = "\t", quote = F, row.names = FALSE)